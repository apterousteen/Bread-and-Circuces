...
namespace AI_stuff
{
    public class BasicUnitAI : MonoBehaviour
    {
        ...
        public void UseCard(Card.Card card, UnitInfo unit)
        {
            ...
            {
                case Card.Card.CardEffect.Damage://confirmed
                    turnManager.AddAction(new Action(ActionType.Attack, Team.Enemy, card.SpellValue));
                    break;

                case Card.Card.CardEffect.PushBackEnemy:
                {
                    turnManager.AddAction(new Action(ActionType.Attack, Team.Enemy, card.SpellValue));
                ...
                case Card.Card.CardEffect.DamageFinisher: // ????? ???????? ????????? numberCard ? ?????? ????? ????(?? ???? ? ??? ?? ?????)
                    turnManager.AddAction(new Action(ActionType.FinisherAttack, Team.Enemy, card.SpellValue));
                    break;

                case Card.Card.CardEffect.Defense:// confirmed
                    unit.defence += card.SpellValue;
                    break;

                case Card.Card.CardEffect.ShieldedDefense:
                {
                    unit.defence += card.SpellValue;
                    if (unit.withShield)
                        unit.defence += 1;
            ...
            switch (card.FirstCardEffTwo)
            {
                ...
                case Card.Card.CardEffect.DamageAfterDiscard:
                ...
                case Card.Card.CardEffect.NearCardDrow:
                    turnManager.AddAction(new Action(ActionType.NearDraw, Team.Enemy, card.SecondSpellValue));
                    break;

                case Card.Card.CardEffect.DiscardEnemy:
                    turnManager.AddAction(new Action(ActionType.DiscardOpponent, Team.Enemy, card.SecondSpellValue));
                    break;

                case Card.Card.CardEffect.ManaAdd:
                {
